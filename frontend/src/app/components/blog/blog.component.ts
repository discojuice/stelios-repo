...
import { BlogPost, GroupedMedia, GroupedPost } from '../../models/blog-post';
...
export class BlogComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  onThumbError(event: Event, media: GroupedMedia): void {
    const img = event.target as HTMLImageElement;

    if (!img.dataset['fallback']) {
      // No generated thumbnail for this file: fall back to the original image.
      img.dataset['fallback'] = '1';
      img.src = media.mediaType === 'image' ? media.mediaUrl : '/video-placeholder.svg';
      return;
    }

    if (img.dataset['fallback'] === 'final') {
      return;
    }

    console.warn('Media failed to load:', media.mediaUrl);
    img.dataset['fallback'] = 'final';
    img.src = '/image-placeholder.svg';
    img.classList.add('loaded', 'errored');
  }

  // Thumbnails are generated by scripts/make-thumbs.py into assets/blog/thumbs/<name>.jpg.
  private thumbUrlFor(mediaUrl: string, posterUrl?: string): string {
    if (posterUrl) {
      return posterUrl;
    }
    const match = mediaUrl.match(/^(.*assets\/blog\/)([^/?#]+)\.[^./?#]+$/);
    return match ? `${match[1]}thumbs/${match[2]}.jpg` : mediaUrl;
  }
  ...
  private groupPosts(posts: BlogPost[]): GroupedPost[] {
    ...
    for (const post of posts) {
      ...
      grouped.media.push({
        ...
        thumbUrl: this.thumbUrlFor(post.mediaUrl, post.posterUrl)
      });
